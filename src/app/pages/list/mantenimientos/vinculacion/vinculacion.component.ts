...
import { ModalVinculacionComponent } from 'src/app/pages/forms/forms-mantenimientos/modal-vinculacion/modal-vinculacion.component';
...
export class VinculacionComponent {
  ...
  page!: number;
  pageSize: number = 10;
  vinculacionList!: Vinculacion[];
  paginador: any;
  nombre: string = 'vinculacion';
  ...
  obtenerListaNoEliminadaOrdenada(sortColumn: string, sortDirection: string): void {
    ...
    const sub: Subscription = this.vinculacionService.getNotDeletedOrdered(this.pageSize, this.page, sortColumn, sortDirection).subscribe({
      next: (response: any) => {
        ...
        this.vinculacionList = response.content as Vinculacion[];
  ...
  /**
   * Método que elimina una vinculacion.
   * @param vinculacion La vinculacion a eliminar.
   */
  delete(vinculacion: Vinculacion): void {
    // Muestra una alerta para confirmar la eliminación de la vinculacion
    ...
    swalWithBootstrapButtons.fire({
      title: '¿Está seguro?',
      text: `¿Está seguro que desea eliminar ${vinculacion.codigo}?`,
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        // Realiza una solicitud al servicio para marcar la vinculacion como eliminada
        const sub = this.vinculacionService.statusEliminado(vinculacion.codigo).subscribe({
          next: response => {
            // Si se elimina correctamente, se filtra la vinculacion de la lista local
            this.vinculacionList = this.vinculacionList.filter(ec => ec !== vinculacion);
            swalWithBootstrapButtons.fire(
              'Elemento eliminado',
              `Elemento ${vinculacion.vinculacionFlota.flotaVehicular.placa} marcado como eliminado con éxito.`,
              'success'
            );
          },
          error: error => {
            // Se maneja el error en caso de que ocurra algún problema al eliminar la vinculacion
            swalWithBootstrapButtons.fire(
              'Error',
  ...
  /**
   * Método que abre el modal para crear o editar una vinculacion.
   * @param vinculacion La vinculacion a editar (opcional).
   */
  public openModal(vinculacion?: Vinculacion): void {
    // Abre el modal para crear o editar una vinculacion
    var dialogRef = this.dialog.open(ModalVinculacionComponent, {
      height:'60%',
    ...
    if (vinculacion != null)
      dialogRef.componentInstance.vinculacion = vinculacion;
    dialogRef.afterClosed().subscribe({
      next: result => {
        console.info('Result:', result);
        if (result) {
          // Refresca el listado de vinculacionList después de crear o editar una vinculacion
          this.obtenerListaNoEliminadaOrdenada(this.sortColumn, this.sortDirection);
        }
      },
      error: error => {
