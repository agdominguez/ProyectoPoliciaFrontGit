...
export class FlotaVehicularService {

  private urlEndPoint: string = 'http://localhost:8080/flotaVehicular'
  ...
  getListGlobal(pageSize: number, page: number): Observable<any> {
    ...
    return this.http.get<any>(url).pipe(
      ...
      tap((response: any) => {
        (response.content as FlotaVehicular[]).forEach(flotaVehicular => {
        });
      })
    );
  }
  ...
  getElement(): Observable<FlotaVehicular[]> {
    return this.http.get<FlotaVehicular[]>(this.urlEndPoint).pipe(
      catchError(error => this.handleError(error))
    );
  }
  ...
  create(flotaVehicular: FlotaVehicular): Observable<any> {
    console.log(this.urlEndPoint, flotaVehicular);
    return this.http.post<any>(this.urlEndPoint, flotaVehicular, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error))
    );
  }
  ...
  update(flotaVehicular: FlotaVehicular): Observable<FlotaVehicular> {
    return this.http.put<any>(`${this.urlEndPoint}/${flotaVehicular.codigo}`, flotaVehicular, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error)),
      map((response: any) => response.elemento as FlotaVehicular)
    );
  }
  ...
  delete(id: any): Observable<FlotaVehicular> {
    return this.http.delete<FlotaVehicular>(`${this.urlEndPoint}/${id}`, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error))
    );
  }
   ...
   */
  getElemtsform(): Observable<FlotaVehicular[]> {
    return this.http.get<FlotaVehicular[]>(this.urlEndPoint + '/list').pipe(
      catchError(error => this.handleError(error))
    );
  }
  /**
  ...
  getNotDeletedOrdered(pageSize: number, page: number, sortColumn: string, sortDirection: string): Observable<any> {
    ...
    return this.http.get<any>(url, { params }).pipe(
      catchError(error => this.handleError(error)),
      tap((response: any) => {
        (response.content as FlotaVehicular[]).forEach(apertura => {
