...
export class SugerenciaService {
  ...
  private urlEndPoint: string = 'http://localhost:8080/sugerencia'
  ...
  getListGlobal(pageSize: number, page: number): Observable<any> {
    ...
    return this.http.get<any>(url).pipe(
      ...
      tap((response: any) => {
        (response.content as Sugerencia[]).forEach(sugerencia => {
        });
      })
    );
  }
  ...
  getElement(): Observable<Sugerencia[]> {
    return this.http.get<Sugerencia[]>(this.urlEndPoint).pipe(
      catchError(error => this.handleError(error))
    );
  }
  ...
  create(sugerencia: Sugerencia): Observable<any> {
    console.log(this.urlEndPoint, sugerencia);
    return this.http.post<any>(this.urlEndPoint, sugerencia, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error))
    );
  }
  createPublic(sugerencia: Sugerencia): Observable<any> {
    console.log(this.urlEndPoint + '/nuevo', sugerencia);
    return this.http.post<any>(this.urlEndPoint + '/nuevo', sugerencia, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error))
    );
  }
  ...
  update(sugerencia: Sugerencia): Observable<Sugerencia> {
    return this.http.put<any>(`${this.urlEndPoint}/${sugerencia.codigo}`, sugerencia, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error)),
      map((response: any) => response.elemento as Sugerencia)
    );
  }
  ...
  delete(id: any): Observable<Sugerencia> {
    return this.http.delete<Sugerencia>(`${this.urlEndPoint}/${id}`, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error))
    );
  }
   ...
   */
  getElemtsform(): Observable<Sugerencia[]> {
    return this.http.get<Sugerencia[]>(this.urlEndPoint + '/list').pipe(
      catchError(error => this.handleError(error))
    );
  }
  getElemtsformPublic(): Observable<Sugerencia[]> {
    return this.http.get<Sugerencia[]>(this.urlEndPoint + '/listado').pipe(
      catchError(error => this.handleError(error))
    );
  }
  /**
  ...
  getNotDeletedOrdered(pageSize: number, page: number, sortColumn: string, sortDirection: string): Observable<any> {
    ...
    return this.http.get<any>(url, { params }).pipe(
      catchError(error => this.handleError(error)),
      tap((response: any) => {
        (response.content as Sugerencia[]).forEach(apertura => {
