...
export class RangoPolicialService {

  private urlEndPoint: string = 'http://localhost:8080/rangoPolicial'
  ...
  getListGlobal(pageSize: number, page: number): Observable<any> {
    ...
    return this.http.get<any>(url).pipe(
      ...
      tap((response: any) => {
        (response.content as RangoPolicial[]).forEach(rangoPolicial => {
        });
      })
    );
  }
  ...
  getElement(): Observable<RangoPolicial[]> {
    return this.http.get<RangoPolicial[]>(this.urlEndPoint).pipe(
      catchError(error => this.handleError(error))
    );
  }
  ...
  create(rangoPolicial: RangoPolicial): Observable<any> {
    console.log(this.urlEndPoint, rangoPolicial);
    return this.http.post<any>(this.urlEndPoint, rangoPolicial, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error))
    );
  }
  ...
  update(rangoPolicial: RangoPolicial): Observable<RangoPolicial> {
    return this.http.put<any>(`${this.urlEndPoint}/${rangoPolicial.codigo}`, rangoPolicial, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error)),
      map((response: any) => response.elemento as RangoPolicial)
    );
  }
  ...
  delete(id: any): Observable<RangoPolicial> {
    return this.http.delete<RangoPolicial>(`${this.urlEndPoint}/${id}`, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error))
    );
  }
   ...
   */
  getElemtsform(): Observable<RangoPolicial[]> {
    return this.http.get<RangoPolicial[]>(this.urlEndPoint + '/list').pipe(
      catchError(error => this.handleError(error))
    );
  }
  /**
  ...
  getNotDeletedOrdered(pageSize: number, page: number, sortColumn: string, sortDirection: string): Observable<any> {
    ...
    return this.http.get<any>(url, { params }).pipe(
      catchError(error => this.handleError(error)),
      tap((response: any) => {
        (response.content as RangoPolicial[]).forEach(apertura => {
