...
import { TipoReclamo } from 'src/app/entities/personas/TipoReclamo';
...
export class TipoReclamoService {
  ...
  private urlEndPoint: string = 'http://localhost:8080/tipoReclamo'
  ...
  getListGlobal(pageSize: number, page: number): Observable<any> {
    ...
    return this.http.get<any>(url).pipe(
      ...
      tap((response: any) => {
        (response.content as TipoReclamo[]).forEach(tipoReclamo => {
        });
      })
    );
  }
  ...
  getElement(): Observable<TipoReclamo[]> {
    return this.http.get<TipoReclamo[]>(this.urlEndPoint).pipe(
      catchError(error => this.handleError(error))
    );
  }
  ...
  create(tipoReclamo: TipoReclamo): Observable<any> {
    console.log(this.urlEndPoint, tipoReclamo);
    return this.http.post<any>(this.urlEndPoint, tipoReclamo, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error))
    );
  }
  ...
  update(tipoReclamo: TipoReclamo): Observable<TipoReclamo> {
    return this.http.put<any>(`${this.urlEndPoint}/${tipoReclamo.codigo}`, tipoReclamo, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error)),
      map((response: any) => response.elemento as TipoReclamo)
    );
  }
  ...
  delete(id: any): Observable<TipoReclamo> {
    return this.http.delete<TipoReclamo>(`${this.urlEndPoint}/${id}`, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error))
    );
  }
   ...
   */
  getElemtsform(): Observable<TipoReclamo[]> {
    return this.http.get<TipoReclamo[]>(this.urlEndPoint + '/list').pipe(
      catchError(error => this.handleError(error))
    );
  }
  getElemtsformPublic(): Observable<TipoReclamo[]> {
    return this.http.get<TipoReclamo[]>(this.urlEndPoint + '/listado').pipe(
      catchError(error => this.handleError(error))
    );
  }
  /**
  ...
  getNotDeletedOrdered(pageSize: number, page: number, sortColumn: string, sortDirection: string): Observable<any> {
    ...
    return this.http.get<any>(url, { params }).pipe(
      catchError(error => this.handleError(error)),
      tap((response: any) => {
        (response.content as TipoReclamo[]).forEach(apertura => {
