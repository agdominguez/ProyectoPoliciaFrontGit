...
export class PersonalPolicialService {

  private urlEndPoint: string = 'http://localhost:8080/personalPolicial'
  ...
  getListGlobal(pageSize: number, page: number): Observable<any> {
    ...
    return this.http.get<any>(url).pipe(
      ...
      tap((response: any) => {
        (response.content as PersonalPolicial[]).forEach(personalPolicial => {
        });
      })
    );
  }
  ...
  getElement(): Observable<PersonalPolicial[]> {
    return this.http.get<PersonalPolicial[]>(this.urlEndPoint).pipe(
      catchError(error => this.handleError(error))
    );
  }
  ...
  create(personalPolicial: PersonalPolicial): Observable<any> {
    console.log(this.urlEndPoint, personalPolicial);
    return this.http.post<any>(this.urlEndPoint, personalPolicial, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error))
    );
  }
  ...
  update(personalPolicial: PersonalPolicial): Observable<PersonalPolicial> {
    return this.http.put<any>(`${this.urlEndPoint}/${personalPolicial.codigo}`, personalPolicial, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error)),
      map((response: any) => response.elemento as PersonalPolicial)
    );
  }
  ...
  delete(id: any): Observable<PersonalPolicial> {
    return this.http.delete<PersonalPolicial>(`${this.urlEndPoint}/${id}`, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error))
    );
  }
   ...
   */
  getElemtsform(): Observable<PersonalPolicial[]> {
    return this.http.get<PersonalPolicial[]>(this.urlEndPoint + '/list').pipe(
      catchError(error => this.handleError(error))
    );
  }
  /**
  ...
  getNotDeletedOrdered(pageSize: number, page: number, sortColumn: string, sortDirection: string): Observable<any> {
    ...
    return this.http.get<any>(url, { params }).pipe(
      catchError(error => this.handleError(error)),
      tap((response: any) => {
        (response.content as PersonalPolicial[]).forEach(apertura => {
