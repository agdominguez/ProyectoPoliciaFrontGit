...
export class VinculacionService {
  ...
  private urlEndPoint: string = 'http://localhost:8080/vinculacion'
  ...
  getListGlobal(pageSize: number, page: number): Observable<any> {
    ...
    return this.http.get<any>(url).pipe(
      ...
      tap((response: any) => {
        (response.content as Vinculacion[]).forEach(vinculacion => {
        });
      })
    );
  }
  ...
  getElement(): Observable<Vinculacion[]> {
    return this.http.get<Vinculacion[]>(this.urlEndPoint).pipe(
      catchError(error => this.handleError(error))
    );
  }
  ...
  create(vinculacion: Vinculacion): Observable<any> {
    console.log(this.urlEndPoint, vinculacion);
    return this.http.post<any>(this.urlEndPoint, vinculacion, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error))
    );
  }
  ...
  update(vinculacion: Vinculacion): Observable<Vinculacion> {
    return this.http.put<any>(`${this.urlEndPoint}/${vinculacion.codigo}`, vinculacion, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error)),
      map((response: any) => response.elemento as Vinculacion)
    );
  }
  ...
  delete(id: any): Observable<Vinculacion> {
    return this.http.delete<Vinculacion>(`${this.urlEndPoint}/${id}`, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error))
    );
  }
   ...
   */
  getElemtsform(): Observable<Vinculacion[]> {
    return this.http.get<Vinculacion[]>(this.urlEndPoint + '/list').pipe(
      catchError(error => this.handleError(error))
    );
  }
  /**
