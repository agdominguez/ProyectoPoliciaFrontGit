...
export class ArmamentoService {
  ...
  private urlEndPoint: string = 'http://localhost:8080/armamento'
  ...
  private handleError(error: HttpErrorResponse): never {
    ...
    if (error.error instanceof ErrorEvent) {
      ...
      if (error.status === 400) {
        errorMessage = error.error.mensaje || 'Armamento incorrecta';
  ...
  getListGlobal(pageSize: number, page: number): Observable<any> {
    ...
    return this.http.get<any>(url).pipe(
      ...
      tap((response: any) => {
        (response.content as Armamento[]).forEach(armamento => {
        });
      })
    );
  }
  ...
  getElement(): Observable<Armamento[]> {
    return this.http.get<Armamento[]>(this.urlEndPoint).pipe(
      catchError(error => this.handleError(error))
    );
  }
  ...
  create(armamento: Armamento): Observable<any> {
  ...
  delete(id: any): Observable<Armamento> {
    return this.http.delete<Armamento>(`${this.urlEndPoint}/${id}`, { headers: this.httpHeaders }).pipe(
      catchError(error => this.handleError(error))
    );
  }
   ...
   */
  getElemtsform(): Observable<Armamento[]> {
    return this.http.get<Armamento[]>(this.urlEndPoint + '/list').pipe(
      catchError(error => this.handleError(error))
    );
  }
  /**
